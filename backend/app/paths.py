...
CONTAINER_OPENCODE_GENERATED_CONFIG = Path("/app/data/opencode.generated.json")


def opencode_generated_config_path() -> Path:
    """OpenCode config output path (Compose /container image / host checkout)."""
    if DOCKER_OPENCODE_GENERATED_CONFIG.parent.is_dir():
        return DOCKER_OPENCODE_GENERATED_CONFIG
    if Path("/app/static").is_dir():
        return CONTAINER_OPENCODE_GENERATED_CONFIG
